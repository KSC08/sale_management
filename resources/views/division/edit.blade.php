                            ...
                            <div class="row">
                                <div class="mb-3 col-md-8">
                                    ...
                                    <input type="text" name="fullName" class="form-control"
                                        value="{{ $divisions->div_fullName }}" placeholder="ป้อนชื่อเติม" required>
                                </div>
                                <div class="form-group col-md-4">
                                    <label style="color:red;">ชื่อย่อส่วนงาน*</label>
                                    <input type="text" name="shortName" class="form-control"
                                        value="{{ $divisions->div_shortName }}" placeholder="ป้อนชื่อย่อ" required>
                                </div>
                            </div>
                            <div class="form-group col-md-6">
                                <label style="color:red;">ภายใต้ฝ่าย*</label>
                                <select class="form-control" name="dep_fullName" required>
                                    <option value="{{ $divisions->dep_id }}">{{ $divisions->dep_fullName }}</option>
                                    @foreach ($dep as $data)
                                        <!--LOOP ข้อมูลใน $department-->
                                        <?php if ($divisions->dep_id != $data->id) { ?>
